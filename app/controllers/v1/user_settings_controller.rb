class V1::UserSettingsController < V1::BaseController
  ...
  def index
    ...
    if email_settings.present?
      user_settings[:email_settings] =  email_settings
    else
      user_settings[:email_settings] = {order_request: Settings.serialize_true,
    ...
    if notification_settings.present?
      ...
      user_settings[:notification_settings] = {order_request: Settings.serialize_true,
        order_processed: Settings.serialize_true, send_order: Settings.serialize_true}
    end
    chatwork_settings = current_user.chatwork_settings
    if chatwork_settings.present?
      user_settings[:chatwork_settings] =  chatwork_settings
    else
      user_settings[:chatwork_settings] = {chatwork_processed: Settings.serialize_true,
